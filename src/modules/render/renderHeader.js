...
export const searchButton = createElement('button', {
   // создаём кнопку и передаём для неё аттрибуты
...
const nav = createElement('div', {
   ...
   parent: container,
   // вторым параметром добавляем кто будет родителем
   // ещё один объект {}, родителем является container
})
...
createElement('ul', {
   ...
   parent: nav,
   // родитель элемента является NAV
   appends: [
      createElement('li', {
   ...
   // указываем детей, через создание LI списка — searchButton, cartLink, favoriteLink
})
